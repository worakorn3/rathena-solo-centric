#!/usr/bin/env python3
"""
tools/gemini/generate_junk_pool.py
----------------------------------
...
Scans db/re/mob_db.yml and db/re/item_db_etc.yml to produce a curated, verified 6-tier
daily bounty pool filtered by monster level, drop rate (>= 10%), and tradeability.
"""
...
def harvest_mob_drops(mob_db_path: str, etc_items: Dict[str, Dict[str, Any]]) -> Dict[int, List[Dict[str, Any]]]:
    ...
    # Flatten and sort top 8 candidates per tier by drop percentage
    ...
    for tier, items in tier_candidates.items():
        ...
        sorted_tiers[tier] = sorted_list[:8]
        print(f"    - Tier {tier} ({TIER_CONFIG[tier]['name']}): Selected {len(sorted_tiers[tier])} top farmable drops.")
...
def build_pure_script_pool(tier_candidates: Dict[int, List[Dict[str, Any]]]) -> Dict[str, Any]:
    """
    Builds the master pool using purely deterministic mathematical formulas.
...
def export_yaml(pool: Dict[str, Any], output_path: str):
    ...
    print(f"[+] Master Junk Pool exported successfully to: {output_path}")


def main():
    parser = argparse.ArgumentParser(description="Script-driven monster drop harvester and Junk Pool generator.")
    parser.add_argument("--output", type=str, default=OUTPUT_YML_PATH, help="Output YAML file path.")
    ...
    export_yaml(pool, args.output)
    print("[+] Done! Zero AI tokens used. 100% farmable, tradeable loot generated.")
